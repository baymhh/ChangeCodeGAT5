...
class GraphAttentionV2Layer(nn.Module):
    def __init__(self, in_features: int, out_features: int, n_heads: int,
        ...
        self.linear_l = nn.Linear(in_features, self.n_hidden * n_heads, bias=False).float()
        if share_weights:
            self.linear_r = self.linear_l  # 共享权重则无需重复转
        else:
            self.linear_r = nn.Linear(in_features, self.n_hidden * n_heads, bias=False).float()

        # 构建三个注意力打分层
        self.attn_ast = nn.Linear(self.n_hidden, 1, bias=False).float()
        self.attn_cfg = nn.Linear(self.n_hidden, 1, bias=False).float()
        self.attn_pdg = nn.Linear(self.n_hidden, 1, bias=False).float()
        ...
        # 投影层，构建Q，多头公用
        self.struct_attn_proj = nn.Linear(in_features, self.n_hidden, bias=False).float()
        # 结构注意力打分层
        self.struct_attn_score = nn.Linear(self.n_hidden, 1, bias=False).float()
        # 结构注意力softmax
        self.struct_softmax = nn.Softmax(dim=-1)

    def forward(self, h: torch.Tensor, ast_adj: torch.Tensor, cfg_adj: torch.Tensor, pdg_adj: torch.Tensor):
        """
        h:   [B, N, F_in]
        ...
        h = h.float()
        ast_adj = ast_adj.float()
        cfg_adj = cfg_adj.float()
        pdg_adj = pdg_adj.float()

        B, N, _ = h.shape
        # 无需再给linear输出加.float()，因为参数已经是double，输出自然是double
        ...
        e_ast = self.attn_ast(self.activation(g_sum)).squeeze(-1)
        e_cfg = self.attn_cfg(self.activation(g_sum)).squeeze(-1)
        e_pdg = self.attn_pdg(self.activation(g_sum)).squeeze(-1)

        if ast_adj.dim() == 3:
            e_ast = e_ast.masked_fill(ast_adj.unsqueeze(-1) == 0, -1e9)
        else:
            e_ast = e_ast.masked_fill(ast_adj == 0, -1e9)

        if cfg_adj.dim() == 3:
            e_cfg = e_cfg.masked_fill(cfg_adj.unsqueeze(-1) == 0, -1e9)
        else:
            e_cfg = e_cfg.masked_fill(cfg_adj == 0, -1e9)

        if pdg_adj.dim() == 3:
            e_pdg = e_pdg.masked_fill(pdg_adj.unsqueeze(-1) == 0, -1e9)
        else:
            e_pdg = e_pdg.masked_fill(pdg_adj == 0, -1e9)

        a_ast = self.softmax(e_ast)
        a_ast = self.dropout(a_ast)

        a_cfg = self.softmax(e_cfg)
        a_cfg = self.dropout(a_cfg)

        a_pdg = self.softmax(e_pdg)
        a_pdg = self.dropout(a_pdg)

        attn_res_ast = torch.einsum('b i j h,b j h f -> b i h f', a_ast, g_r)
        attn_res_cfg = torch.einsum('b i j h,b j h f -> b i h f', a_cfg, g_r)
        attn_res_pdg = torch.einsum('b i j h,b j h f -> b i h f', a_pdg, g_r)

        # 把这三种表示当作其更新前节点的3个邻居，来计算注意力分数，
        # 然后softmax后加权求和得到节点最终表示
        # 先构建多头公用Q，把原始节点特征投影一下
        h_proj = self.struct_attn_proj(h)
        # 激活一下
        h_proj_activated = self.activation(h_proj)

        # 扩展三个子表示，方便计算
        attn_res_ast_unsq = attn_res_ast.unsqueeze(3)
        attn_res_cfg_unsq = attn_res_cfg.unsqueeze(3)  # 形状：[B, N, n_heads, 1, n_hidden]
        attn_res_pdg_unsq = attn_res_pdg.unsqueeze(3)

        # 拼接：[B, N, n_heads, 3, n_hidden]
        attn_res_concat = torch.cat([attn_res_ast_unsq, attn_res_cfg_unsq, attn_res_pdg_unsq], dim=3)

        # Q的扩展，方便计算：[B, N, n_hidden] → [B, N, 1, 1, n_hidden]
        h_proj_expanded = h_proj_activated.unsqueeze(2).unsqueeze(2)

        # 关联，准备计算注意力：[B, N, n_heads, 3, n_hidden]
        struct_combined = attn_res_concat + h_proj_expanded

        # 计算每种结构的注意力分数：[B, N, n_heads, 3, n_hidden] → [B, N, n_heads, 3, 1] → [B, N, n_heads, 3]
        struct_scores = self.struct_attn_score(struct_combined)
        struct_scores = struct_scores.squeeze(-1)

        # 归一化、正则化
        struct_weights = self.struct_softmax(struct_scores)
        struct_weights = self.dropout(struct_weights)

        # 权重的扩展，准备加权求和：[B, N, n_heads, 3] → [B, N, n_heads, 3, 1]
        struct_weights_unsq = struct_weights.unsqueeze(-1)

        # 三种表示×对应权重：[B, N, n_heads, 3, n_hidden]
        struct_weighted = attn_res_concat * struct_weights_unsq

        # 三种表示求和：[B, N, n_heads, n_hidden]
        # 其实这里也可以加个残差连接（但目前没加）
        attn_res = torch.sum(struct_weighted, dim=3)
...
class GraphEmbedding(nn.Module):
    def __init__(self, feature_dim_size, hidden_size, dropout, act=nn.functional.relu):
        ...
        self.graphAttv2 = GraphAttentionV2Layer(feature_dim_size, hidden_size, n_heads=1, is_concat=True, dropout=0.3, leaky_relu_negative_slope=0.2, share_weights=False)
    ...
    def forward(self, inputs, ast_adj, cfg_adj, pdg_adj, mask):
        x = inputs
        # 先广播一下，把[B, M]变为[B, M, 1]
        mask = mask.unsqueeze(-1)
        #x = self.gnn(x, adj) * mask  # Residual Connection, can use a weighted sum
        # x = self.graphAtt(x, adj) * mask
        x = self.graphAttv2(x, ast_adj, cfg_adj, pdg_adj) * mask
        # soft attention
